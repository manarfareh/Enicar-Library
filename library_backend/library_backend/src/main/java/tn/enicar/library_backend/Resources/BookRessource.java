...
public class BookRessource {
    ...
    @PostMapping("/addDigitalArtical")
    public ResponseEntity<?> addDigitalArtical(@RequestBody Book book) {

       book = new Article(book.getTitle(), book.getAuthor(),1,book.getUrl(), book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());
       // Save the book object to the database
        bookRepo.save(book);
        bookService.addDigitalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addDigitalPfeBook")
    public ResponseEntity<?> addDigitalPfeBook(@RequestBody PfeBook book) {
        book = new PfeBook(book.getTitle(), book.getAuthor(),1,book.getUrl(), book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());

        bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addDigitalExam")
    public ResponseEntity<?> addDigitalExam(@RequestBody Exam book) {
        book = new Exam(book.getTitle(), book.getAuthor(),1,book.getUrl(), book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable(),book.getSubject(),book.getAClass());
        bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addDigitalBook")
    public ResponseEntity<?> addDigitalBook(@RequestBody Book book) {
        book = new Book(book.getTitle(), book.getAuthor(),1,book.getUrl(), book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());
        bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addPhysicalArtical")
    public ResponseEntity<?> addPhysicalArtical(@RequestBody Book book) {

        book = new Article(book.getTitle(), book.getAuthor(),book.getIsDigit(),book.getUrl(), book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());
        // Save the book object to the database
        bookRepo.save(book);
        bookService.addDigitalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addPhysicalPfeBook")
    public ResponseEntity<?> addPhysicalPfeBook(@RequestBody Book book) {
        book = new PfeBook(book.getTitle(), book.getAuthor(),0,null, book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());
         bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addPhysicalExam")
    public ResponseEntity<?> addPhysicalExam(@RequestBody Exam book) {
        book = new Exam(book.getTitle(), book.getAuthor(),0,null, book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable(),book.getSubject(),book.getAClass());
        bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/addPhysicalBook")
    public ResponseEntity<?> addPhysicalBook(@RequestBody Book book) {
        book = new Book(book.getTitle(), book.getAuthor(),0,null, book.getPublicationYear(), book.getLanguage(), book.getPageCount(), book.getDescription(), book.getIsAvailable());
        bookService.addPhysicalBook(book);
        return ResponseEntity.ok().build();
    }
    @GetMapping("/find/{id}")
